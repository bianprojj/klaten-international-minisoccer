...
export default function VenueFeatureManager() {
  ...
  async function save() { const endpoint = editingId ? `/api/admin/features/${editingId}` : "/api/admin/features"; const { res: response, data: __body } = await fetchJson(endpoint, { method: editingId ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }); const body = __body; setMessage(String(body.message ?? "") ?? (response.ok ? "Fasilitas tersimpan." : "Gagal menyimpan.")); if (response.ok) { setForm({ name: "", description: "", imageUrl: "" }); setEditingId(null); await load(); } }
