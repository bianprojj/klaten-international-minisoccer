...
export default function VenueFeatureManager({ canManage, canRead }: { canManage: boolean; canRead: boolean }) {
  ...
  async function load() { if (!canRead && !canManage) return; const { res: response, data: __body } = await fetchJson("/api/admin/features", { cache: "no-store" }); const body = __body; if (response.ok) setFeatures(body.data ?? []); else setMessage(String(body.message ?? "") ?? "Unable to load features."); }
  useEffect(() => { void load(); }, []);
