...
export default function StaffPaymentViewer({ adminName, useMain = true }: { adminName: string; useMain?: boolean }) {
  ...
  const content = (
    <div className="mx-auto max-w-7xl space-y-6 overflow-x-clip px-4 sm:px-6 lg:px-8" id="staff-payments">
              ...
              {payments.map((payment) => (
                <div key={payment.id} className="rounded-2xl border border-white/10 bg-[color:rgba(255,255,255,0.02)] p-4">
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    <button onClick={() => handleDeletePayment(payment.id)} className="flex-1 rounded-full border border-rose-500/20 px-3 py-2 text-sm text-rose-300">Delete</button>
                  </div>
                </div>
              ))}
              {payments.length === 0 ? (
